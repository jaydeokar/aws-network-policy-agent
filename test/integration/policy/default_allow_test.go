...
var _ = Describe("Default Allow Mode Test Cases", func() {
	Context("When a pod is first launched and traffic starts flowing", func() {
		...
		It("if connection is made before network policy is applied the response packet should be accepted and never denied when applying ingress network policy", func() {
			...
			By("Check if the all response is accepted", func() {
				time.Sleep(2 * time.Minute)
				logs, err := fw.PodManager.PodLogs(namespace, podName)
				Expect(err).ToNot(HaveOccurred())
				err = processDefaultAllowLogs(logs, true, false)
		...
		It("if egress connection is made before network policy is applied the response packet should be accepted and later denied (egress network policy)", func() {
			By("Creating a network policy which only allows egress from a different client", func() {
			...
			By("Check if the first response is accepted and following requests from client is denied", func() {
				time.Sleep(2 * time.Minute)
				logs, err := fw.PodManager.PodLogs(namespace, podName)
				Expect(err).ToNot(HaveOccurred())
...
func processDefaultAllowLogs(podlogs string, allAllow bool, deniedAfterAllow bool) error {
	...
	if !passFlag && !failFlag {
		...
		return fmt.Errorf("Failed as traffic probes did not get ACCEPT first and then DENY")
	}
