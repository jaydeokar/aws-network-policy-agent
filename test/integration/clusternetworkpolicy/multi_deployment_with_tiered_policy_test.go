	...
	Context("When multiple deployments across namespaces with tiered policies", func() {
		...
		It("should enforce tiered cluster network policies correctly", func() {
			...
			By("Verifying network policy enforcement", func() {
				...
				By("Testing same namespace communication (ns1 to ns1)", func() {
					...
					result, err := fw.PodManager.ExecInPod("ns1", app1Pods[0].Name, []string{"wget", "-qO-", "--timeout=5", fmt.Sprintf("http://%s:8080", app2Pods[0].Status.PodIP)})
					Expect(err).ToNot(HaveOccurred())
				...
				By("Testing ns2 to ns3 communication (allowed by policy)", func() {
					...
					result, err := fw.PodManager.ExecInPod("ns2", app3Pods[0].Name, []string{"wget", "-qO-", "--timeout=5", fmt.Sprintf("http://%s:8080", app4Pods[0].Status.PodIP)})
					Expect(err).ToNot(HaveOccurred())
					Expect(result).To(ContainSubstring("Server Response"))
				})
				...
				By("Testing ns1 to ns2 communication (should be denied)", func() {
					...
					_, err := fw.PodManager.ExecInPod("ns1", app1Pods[0].Name, []string{"wget", "-qO-", "--timeout=5", fmt.Sprintf("http://%s:8080", app3Pods[0].Status.PodIP)})
				...
				By("Testing ns3 to ns2 communication (should be denied)", func() {
					...
					_, err := fw.PodManager.ExecInPod("ns3", app4Pods[0].Name, []string{"wget", "-qO-", "--timeout=5", fmt.Sprintf("http://%s:8080", app3Pods[0].Status.PodIP)})
					Expect(err).To(HaveOccurred())
				})
				...
				By("Testing ns4 to ns1 communication (denied by baseline tier)", func() {
					...
					_, err := fw.PodManager.ExecInPod("ns4", app5Pods[0].Name, []string{"wget", "-qO-", "--timeout=5", fmt.Sprintf("http://%s:8080", app1Pods[0].Status.PodIP)})
					Expect(err).To(HaveOccurred())
				})

				By("Testing ns4 to ns2 communication (denied by baseline tier)", func() {
					fmt.Printf("Src IP: %s Dest IP: %s Dest Port: %d Proto TCP Verdict\n", app5Pods[0].Status.PodIP, app3Pods[0].Status.PodIP, 8080)
					_, err := fw.PodManager.ExecInPod("ns4", app5Pods[0].Name, []string{"wget", "-qO-", "--timeout=5", fmt.Sprintf("http://%s:8080", app3Pods[0].Status.PodIP)})
					Expect(err).To(HaveOccurred())
				})
				...
				By("Testing ns1 to ns4 communication (denied by baseline tier)", func() {
					...
					_, err := fw.PodManager.ExecInPod("ns1", app1Pods[0].Name, []string{"wget", "-qO-", "--timeout=5", fmt.Sprintf("http://%s:8080", app5Pods[0].Status.PodIP)})
					Expect(err).To(HaveOccurred())
				})
			})
		})
		...
		It("should allow DNS resolution while denying ns2 traffic", func() {
			...
			By("Testing that other traffic from ns1 for ingress and egress (should be denied)", func() {
				...
				_, err = fw.PodManager.ExecInPod("ns2", app3Pods[0].Name, []string{"wget", "-qO-", "--timeout=5", fmt.Sprintf("http://%s:8080", app1Pods[0].Status.PodIP)})
				Expect(err).To(HaveOccurred())
			})
		})
